...
#define PIN1 4                                                          //define pin 15 for Neopixel LED
#define PIN2 5
#define PIN3 6
#define PIN4 7
...
const char* ssid = "Verizon-SM-G935V-656D";
const char* pass = "zstandley";
...
const int glowfade = 1;                                                 //determines how fast the color changes
int maxtD = 15;                                                         //temperature range
int maxwD = 20;                                                         //humidity range
...
const char* SHIFTR_MKR_02_Temperature = "/02_MKR_Temperature";          //Arduino MKR1000 incoming messages to shiftr.io
const char* SHIFTR_MKR_02_Wind = "/02_MKR_Wind";                        //Arduino MKR1000 incoming messages to shiftr.io
const char* SHIFTR_MKR_03_Temperature = "/03_MKR_Temperature";          //Arduino MKR1000 incoming messages to shiftr.io
const char* SHIFTR_MKR_03_Wind = "/03_MKR_Wind";                        //Arduino MKR1000 incoming messages to shiftr.io
const char* SHIFTR_MKR_04_Temperature = "/04_MKR_Temperature";          //Arduino MKR1000 incoming messages to shiftr.io
const char* SHIFTR_MKR_04_Wind = "/04_MKR_Wind";                        //Arduino MKR1000 incoming messages to shiftr.io
const char* SHIFTR_MKR_DT = "/MKR_DT";    

const int pause = 200;                                                  //delay amount in milliseconds
...
void loop() {
  ...
  if (!client.connected()) 
  {
    connect(SHIFTR_OWM_Temperature, SHIFTR_OWM_Humidity, SHIFTR_MKR_01_Temperature, SHIFTR_MKR_01_Wind, SHIFTR_MKR_02_Temperature, SHIFTR_MKR_02_Wind, SHIFTR_MKR_03_Temperature, SHIFTR_MKR_03_Wind, SHIFTR_MKR_04_Temperature, SHIFTR_MKR_04_Wind);
  }
  t = millis();
  if (t - lastT > pause) //TIME
  {
    ...
    client.publish(SHIFTR_MKR_DT, String(temperatureD_01));
    ...
    windcolor(RevP_Wind_01, colorT01a, colorT01);
    ...
    Serial.println();
    windcolor(RevP_Wind_02, colorT02a, colorT02);
    windcolor(RevP_Wind_03, colorT03a, colorT03);
    windcolor(RevP_Wind_04, colorT04a, colorT04);
    ...
    for (int j = 0; j < LEDSS; j++) //what strip
    {
      ...
      { 
        ...
        if (j == 2)
        {
          if(invertX)
          {
            strip2.setPixelColor(i, LEDR[j][i+55], LEDG[j][i+55], LEDB[j][i+55]);      //[i+numpix to skip] for doorway. nix if running full strip.
            strip2.show();
          }
          else
          {
            strip3.setPixelColor(i, LEDR[j][i+55], LEDG[j][i+55], LEDB[j][i+55]);      //[i+numpix to skip] for doorway. nix if running full strip.
            strip3.show();
          }
        }
        if (j == 3)
        {
          if(invertX)
          {
            strip1.setPixelColor(i, LEDR[j][i+55], LEDG[j][i+55], LEDB[j][i+55]);       //[i+numpix to skip] for doorway. nix if running full strip.
            strip1.show();
          }
          else
          {
            strip4.setPixelColor(i, LEDR[j][i+55], LEDG[j][i+55], LEDB[j][i+55]);       //[i+numpix to skip] for doorway. nix if running full strip.
            strip4.show();
          }
        }
